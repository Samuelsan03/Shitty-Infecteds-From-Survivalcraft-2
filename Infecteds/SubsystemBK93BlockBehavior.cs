...
namespace Game
{
	public class SubsystemBK93BlockBehavior : SubsystemBlockBehavior
	{
		...
		private int m_bk93BlockIndex;
		private int m_bk93AmmunitionBlockIndex;
		...
		public override void Load(ValuesDictionary valuesDictionary)
		{
			...
			m_bk93BlockIndex = BlocksManager.GetBlockIndex<BK93Block>(false, false);
			m_bk93AmmunitionBlockIndex = BlocksManager.GetBlockIndex<BK93AmmunitionBlock>(false, false);
		...
		public override bool OnAim(Ray3 aim, ComponentMiner componentMiner, AimState state)
		{
			...
			if (inventory != null)
			{
				...
				if (activeSlotIndex >= 0)
				{
					...
					if (num == m_bk93BlockIndex && slotCount > 0)
					{
						double gameTime;
						...
						BK93Block.LoadState loadState = BK93Block.GetLoadState(data);
						int ammoCount = BK93Block.GetAmmoCount(data);
							...
							case AimState.InProgress:
								{
									...
									if (loadState == BK93Block.LoadState.Loaded && ammoCount > 0)
									{
										...
										if (!alreadyFiredThisAim && timeSinceLastFire >= FireRate)
										{
											...
											for (int i = 0; i < PelletCount; i++)
											{
												...
												int bulletValue = Terrain.MakeBlockValue(m_bulletBlockIndex, 0, FirearmsBulletBlock.SetFirearmsBulletType(0, FirearmsBulletBlock.FirearmsBulletType.BK93Bullet));
											...
											m_subsystemAudio.PlaySound("Audio/Armas/bk 43", 1f, m_random.Float(-0.1f, 0.1f), vector, 12f, true);
											...
											int newData = BK93Block.SetAmmoCount(Terrain.ExtractData(num2), newAmmoCount);

											if (newAmmoCount <= 0)
											{
												newData = BK93Block.SetLoadState(newData, BK93Block.LoadState.Empty);
												loadState = BK93Block.LoadState.Empty;
											}
									...
									else
									{
										...
										{
											string ammoName = LanguageControl.GetBlock("BK93AmmunitionBlock", "DisplayName");
		...
		public override int GetProcessInventoryItemCapacity(IInventory inventory, int slotIndex, int value)
		{
			...
			if (slotContents != m_bk93BlockIndex) return 0;

			int ammoCount = BK93Block.GetAmmoCount(Terrain.ExtractData(inventory.GetSlotValue(slotIndex)));

			if (ammoCount >= MaxAmmo) return 0;

			int itemContents = Terrain.ExtractContents(value);
			if (itemContents == m_bk93AmmunitionBlockIndex)
				return 1;
		...
		public override void ProcessInventoryItem(IInventory inventory, int slotIndex, int value, int count, int processCount, out int processedValue, out int processedCount)
		{
			...
			if (processCount == 1)
			{
				...
				int newData = BK93Block.SetLoadState(data, BK93Block.LoadState.Loaded);
				newData = BK93Block.SetAmmoCount(newData, MaxAmmo);

				processedValue = 0;
				processedCount = 0;
				inventory.RemoveSlotItems(slotIndex, 1);
				inventory.AddSlotItems(slotIndex, Terrain.MakeBlockValue(m_bk93BlockIndex, 0, newData), 1);
