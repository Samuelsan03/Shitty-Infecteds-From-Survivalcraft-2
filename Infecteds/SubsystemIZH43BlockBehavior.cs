...
namespace Game
{
	public class SubsystemIZH43BlockBehavior : SubsystemBlockBehavior
	{
		...
		private int m_izh43AmmunitionBlockIndex;
		...
		public override bool OnAim(Ray3 aim, ComponentMiner componentMiner, AimState state)
		{
			...
			if (inventory != null)
			{
				...
				if (activeSlotIndex >= 0)
				{
					...
					if (num == m_izh43BlockIndex && slotCount > 0)
					{
						double gameTime;
						...
						IZH43Block.LoadState loadState = IZH43Block.GetLoadState(data);
						int ammoCount = IZH43Block.GetAmmoCount(data);
							...
							case AimState.InProgress:
								{
									...
									if (loadState == IZH43Block.LoadState.Loaded && ammoCount > 0)
									{
										...
										if (!alreadyFiredThisAim && timeSinceLastFire >= FireRate)
										{
											...
											for (int i = 0; i < PelletCount; i++)
											{
												...
												int bulletValue = Terrain.MakeBlockValue(m_bulletBlockIndex, 0, FirearmsBulletBlock.SetFirearmsBulletType(0, FirearmsBulletBlock.FirearmsBulletType.IZH43Bullet));
											...
											m_subsystemAudio.PlaySound("Audio/Armas/izh 43 fuego", 1f, m_random.Float(-0.1f, 0.1f), vector, 12f, true);
											...
											int newData = IZH43Block.SetAmmoCount(Terrain.ExtractData(num2), newAmmoCount);

											if (newAmmoCount <= 0)
											{
												newData = IZH43Block.SetLoadState(newData, IZH43Block.LoadState.Empty);
												loadState = IZH43Block.LoadState.Empty;
											}
		...
		public override int GetProcessInventoryItemCapacity(IInventory inventory, int slotIndex, int value)
		{
			...
			if (slotContents != m_izh43BlockIndex) return 0;

			int ammoCount = IZH43Block.GetAmmoCount(Terrain.ExtractData(inventory.GetSlotValue(slotIndex)));

			if (ammoCount >= MaxAmmo) return 0;

			int itemContents = Terrain.ExtractContents(value);
			if (itemContents == m_izh43AmmunitionBlockIndex)
				return 1;
		...
		public override void ProcessInventoryItem(IInventory inventory, int slotIndex, int value, int count, int processCount, out int processedValue, out int processedCount)
		{
			...
			if (processCount == 1)
			{
				...
				int newData = IZH43Block.SetLoadState(data, IZH43Block.LoadState.Loaded);
				newData = IZH43Block.SetAmmoCount(newData, MaxAmmo);

				processedValue = 0;
				processedCount = 0;
				inventory.RemoveSlotItems(slotIndex, 1);
				inventory.AddSlotItems(slotIndex, Terrain.MakeBlockValue(m_izh43BlockIndex, 0, newData), 1);
