...
namespace Game
{
	public class DesertEagleBlock : Block
	{
		...
		{
			m_texture = ContentManager.Get<Texture2D>("Textures/Armas De Mierda/deserteagle");

			Model model = ContentManager.Get<Model>("Models/Armas De Porqueria/deserteagle");

			ModelMesh mainMesh = model.FindMesh("deserteagle", true);
		...
		// Bits 1-4: Cantidad de balas (0-7) - Desert Eagle usa cargador de 7 balas
		...
		{
			return (data & ~0x1E) | ((Math.Clamp(count, 0, 7) & 0xF) << 1);
		}
