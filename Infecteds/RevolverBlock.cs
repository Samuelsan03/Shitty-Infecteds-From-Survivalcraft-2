...
namespace Game
{
	public class RevolverBlock : Block
	{
		...
		{
			m_texture = ContentManager.Get<Texture2D>("Textures/Armas De Mierda/Revolver");

			Model model = ContentManager.Get<Model>("Models/Armas De Porqueria/Revolver");

			// El DAE tiene el mesh llamado "meshId0_name.014"
		...
		// Bits 1-4: Cantidad de balas (0-6) - Revolver usa cilindro de 6 balas
		...
		{
			return (data & ~0x1E) | ((Math.Clamp(count, 0, 6) & 0xF) << 1);
		}
