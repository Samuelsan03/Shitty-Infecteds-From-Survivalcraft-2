...
namespace Game
{
	public class Master308Block : Block
	{
		...
		{
			m_texture = ContentManager.Get<Texture2D>("Textures/Armas De Mierda/GUN");

			Model model = ContentManager.Get<Model>("Models/Armas De Porqueria/Master 308");

			// Malla principal del arma
			ModelMesh mainMesh = model.FindMesh("Musket", true);
			Matrix mainTransform = BlockMesh.GetBoneAbsoluteTransform(mainMesh.ParentBone);

			// Malla del martillo/percutor
			ModelMesh hammerMesh = model.FindMesh("Hammer", true);
			Matrix hammerTransform = BlockMesh.GetBoneAbsoluteTransform(hammerMesh.ParentBone);

			// Estado descargado - martillo en posición normal
			...
			m_standaloneBlockMeshUnloaded.AppendModelMeshPart(hammerMesh.MeshParts[0], hammerTransform, false, false, false, false, Color.White);

			// Estado cargado - martillo amartillado (rotado)
			Matrix hammerLoadedTransform = hammerTransform * Matrix.CreateRotationX(-0.6f);
			...
			m_standaloneBlockMeshLoaded.AppendModelMeshPart(hammerMesh.MeshParts[0], hammerLoadedTransform, false, false, false, false, Color.White);
		...
		// Bits 1-5: Cantidad de balas (0-5 para Master 308, cargador interno bolt-action)
		...
		{
			return (data & ~0x3E) | ((Math.Clamp(count, 0, 5) & 0x1F) << 1);
		}
