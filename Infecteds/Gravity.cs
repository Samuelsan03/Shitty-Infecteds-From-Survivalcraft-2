...
namespace Game
{
	...
	public class Gravity : ComponentBehavior, IUpdateable
	{
		...
		private ComponentMiner m_componentMiner; // Solo para usar Raycast
		...
		// Parámetros para la detección de impacto (igual que en ComponentChaseBehavior)
		private float m_maxAttackRange = 1.75f;
		private bool m_allowAttackingStandingOnBody = true;
		...
		public override void Load(ValuesDictionary valuesDictionary, IdToEntityMap idToEntityMap)
		{
			...
			m_componentMiner = Entity.FindComponent<ComponentMiner>(true); // Necesario para Raycast
			...
			m_stateMachine.AddState("Chasing",
				...
				update: () =>
				{
					...
					if (m_componentCreatureModel.IsAttackHitMoment && !m_pushAppliedThisHit)
					{
						...
						{
							// Verificar si el golpe realmente impacta (igual que en ComponentChaseBehavior)
							Vector3 hitPoint;
		...
		// ---------------------------------------------------------------------
		// Lógica de detección de impacto (copiada de ComponentChaseBehavior.GetHitBody)
		// SOLO verifica si el golpe impacta, NO aplica daño
		// ---------------------------------------------------------------------
		private ComponentBody GetHitBody(ComponentBody target, out Vector3 hitPoint)
		{
			...
			Vector3 start = m_componentBody.BoundingBox.Center();
			Vector3 end = target.BoundingBox.Center();
			Ray3 ray = new Ray3(start, Vector3.Normalize(end - start));

			// Usamos Raycast de ComponentMiner (sin aplicar daño)
			BodyRaycastResult? bodyRaycastResult = m_componentMiner.Raycast<BodyRaycastResult>(
				ray, RaycastMode.Interaction, true, true, true, null);

			if (bodyRaycastResult != null &&
				bodyRaycastResult.Value.Distance < m_maxAttackRange &&
				(bodyRaycastResult.Value.ComponentBody == target ||
				 ...
				 (target.StandingOnBody == bodyRaycastResult.Value.ComponentBody && m_allowAttackingStandingOnBody)))
			{
		...
		private void ApplyPush(ComponentCreature target)
		{
			...
			float impulseMagnitude = m_pushForce * 1e+9f;
