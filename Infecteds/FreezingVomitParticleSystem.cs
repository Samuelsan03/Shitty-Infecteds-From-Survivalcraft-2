...
namespace Game
{
	public class FreezingVomitParticleSystem : ParticleSystem<FreezingVomitParticleSystem.Particle>
	{
		...
		public FreezingVomitParticleSystem(SubsystemTerrain terrain, SubsystemBodies bodies, SubsystemTime time) : base(120)
		{
		...
		public override bool Simulate(float dt)
		{
			...
			{
				FreezingVomitParticleSystem.Particle particle = base.Particles[i];
				...
				if (particle.IsActive)
				{
					...
					if (particle.Time <= particle.Duration)
					{
						...
						if (!particle.IsStuck)
						{
							...
							{
								if (CheckBodyCollisionAndInfectFlu(newPos, particle))
								{
									particle.IsStuck = true;
									particle.Velocity = Vector3.Zero;
								}
		...
		private bool CheckBodyCollisionAndInfectFlu(Vector3 position, FreezingVomitParticleSystem.Particle particle)
		{
