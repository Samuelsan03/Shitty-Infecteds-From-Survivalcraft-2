...
namespace Game
{
	public class SubsystemM4BlockBehavior : SubsystemBlockBehavior
	{
		...
		private int m_m4AmmunitionBlockIndex;

		private const float FireRate = 0.08f; // M4 es más rápido que AK47
		private const int MaxAmmo = 30;
		private const float EmptySoundCooldown = 0.5f;
		private const float EmptyMessageCooldown = 0.5f;
		private const float MuzzleOffset = 0.85f;
		...
		public override void Load(ValuesDictionary valuesDictionary)
		{
			...
			m_m4AmmunitionBlockIndex = BlocksManager.GetBlockIndex<M4AmmunitionBlock>(false, false);
		...
		public override bool OnAim(Ray3 aim, ComponentMiner componentMiner, AimState state)
		{
			...
			if (inventory != null)
			{
				...
				if (activeSlotIndex >= 0)
				{
					...
					if (num == m_m4BlockIndex && slotCount > 0)
					{
						double gameTime;
						...
						// M4 tiene menos dispersión que AK47 (base más baja)
						Vector3 v = ((componentMiner.ComponentCreature.ComponentBody.IsCrouching ? 0.008f : 0.02f) + 0.12f * MathUtils.Saturate(num4 / 5f)) * new Vector3
						{
						...
						M4Block.LoadState loadState = M4Block.GetLoadState(data);
						int ammoCount = M4Block.GetAmmoCount(data);
							...
							case AimState.InProgress:
								{
									...
									if (loadState == M4Block.LoadState.Loaded && ammoCount > 0)
									{
										...
										if (timeSinceLastFire >= FireRate)
										{
											Vector3 vector = componentMiner.ComponentCreature.ComponentCreatureModel.EyePosition
												+ componentMiner.ComponentCreature.ComponentBody.Matrix.Right * 0.28f
												- componentMiner.ComponentCreature.ComponentBody.Matrix.Up * 0.18f
												+ aim.Direction * MuzzleOffset;
											Vector3 vector2 = aim.Direction;

											int bulletValue = Terrain.MakeBlockValue(m_bulletBlockIndex, 0, FirearmsBulletBlock.SetFirearmsBulletType(0, FirearmsBulletBlock.FirearmsBulletType.M4Bullet));
											// M4 tiene mayor velocidad de bala que AK47
											Vector3 velocity = componentMiner.ComponentCreature.ComponentBody.Velocity + 130f * vector2;
											...
											m_subsystemAudio.PlaySound("Audio/Armas/M4 fuego", 1f, m_random.Float(-0.1f, 0.1f), vector, 10f, true);
											m_subsystemParticles.AddParticleSystem(new TestGunFireParticleSystem(m_subsystemTerrain, vector, vector2), false);
											m_subsystemNoise.MakeNoise(vector, 0.9f, 35f);

											int newAmmoCount = ammoCount - 1;
											int newData = M4Block.SetAmmoCount(Terrain.ExtractData(num2), newAmmoCount);

											if (newAmmoCount <= 0)
											{
												newData = M4Block.SetLoadState(newData, M4Block.LoadState.Empty);
											}
									...
									else
									{
										...
										{
											string ammoName = LanguageControl.GetBlock("M4AmmunitionBlock", "DisplayName");
									...
									if (componentFirstPersonModel != null)
									{
										...
										componentFirstPersonModel.ItemOffsetOrder = new Vector3(-0.19f, 0.14f, 0.07f);
										componentFirstPersonModel.ItemRotationOrder = new Vector3(-0.65f, 0f, 0f);
									}
									componentMiner.ComponentCreature.ComponentCreatureModel.AimHandAngleOrder = 1.35f;
									componentMiner.ComponentCreature.ComponentCreatureModel.InHandItemOffsetOrder = new Vector3(-0.07f, -0.07f, 0.06f);
									componentMiner.ComponentCreature.ComponentCreatureModel.InHandItemRotationOrder = new Vector3(-1.65f, 0f, 0f);
									break;
								}
		...
		public override int GetProcessInventoryItemCapacity(IInventory inventory, int slotIndex, int value)
		{
			...
			if (slotContents != m_m4BlockIndex) return 0;

			int ammoCount = M4Block.GetAmmoCount(Terrain.ExtractData(inventory.GetSlotValue(slotIndex)));

			if (ammoCount >= MaxAmmo) return 0;

			int itemContents = Terrain.ExtractContents(value);
			if (itemContents == m_m4AmmunitionBlockIndex)
				return 1;
		...
		public override void ProcessInventoryItem(IInventory inventory, int slotIndex, int value, int count, int processCount, out int processedValue, out int processedCount)
		{
			...
			if (processCount == 1)
			{
				...
				int newData = M4Block.SetLoadState(data, M4Block.LoadState.Loaded);
				newData = M4Block.SetAmmoCount(newData, MaxAmmo);

				processedValue = 0;
				processedCount = 0;
				inventory.RemoveSlotItems(slotIndex, 1);
				inventory.AddSlotItems(slotIndex, Terrain.MakeBlockValue(m_m4BlockIndex, 0, newData), 1);
